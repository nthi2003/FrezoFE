...
const GlobalLoading = ({ onFinished }: { onFinished: () => void }) => {
  ...
  useGSAP(() => {
    ...
    tl.fromTo('.seed',
      ...
      // Seed burying
      .to('.seed', { y: 10, opacity: 0, duration: 0.4, ease: 'power2.in' }, '+=0.2')
      // Sprout rising
      .fromTo('.sprout',
        { scaleY: 0, opacity: 0, transformOrigin: 'bottom' },
        { scaleY: 1, opacity: 1, duration: 1, ease: 'elastic.out(1, 0.5)' }
      )
      // Final burst
      .fromTo('.leaves',
        { scale: 0, opacity: 0 },
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[9999] bg-white flex items-center justify-center overflow-hidden">
      <div className="relative flex flex-col items-center">
        {/* The Ground */}
        <div className="absolute bottom-[-10px] w-32 h-2 bg-amber-900/10 rounded-full blur-md"></div>

        {/* Seed */}
        <div className="seed w-4 h-5 bg-amber-800 rounded-full shadow-inner mb-2"></div>

        {/* Sprout Structure */}
        <div className="sprout relative flex flex-col items-center">
          <div className="w-1.5 h-16 bg-green-500 rounded-full"></div>
          <div className="leaves absolute -top-2 -left-6 transform -rotate-45">
            <Leaf size={28} className="text-green-600" fill="currentColor" />
          </div>
          <div className="leaves absolute -top-2 -right-6 transform rotate-45 scale-x-[-1]">
            <Leaf size={28} className="text-green-500" fill="currentColor" />
          </div>
        </div>

        <div className="mt-12">
          <p className="font-serif text-farm-primary-dark font-bold tracking-widest animate-pulse">THE FARM HOUSE</p>
          <p className="text-[10px] text-gray-400 text-center mt-1">Đang gieo mầm hy vọng...</p>
        </div>
      </div>
    </div>
  );
...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 bg-white/95 backdrop-blur-md z-50 shadow-sm border-b border-gray-100">
      <div className="bg-farm-primary-dark text-white/90 text-xs py-2 hidden md:block">
      ...
      <div className="container mx-auto px-4 h-20 flex justify-between items-center">
        <div className="flex items-center gap-2 z-50">
          <Link to="/" className="flex items-center gap-2">
            <Leaf size={32} className="text-farm-primary" fill="currentColor" fillOpacity={0.2} />
            <span className="font-serif font-bold text-2xl uppercase tracking-tight text-farm-primary-dark">
              {config.brandName}
            </span>
          </Link>
        </div>
          ...
          {[
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`font-medium text-sm transition-colors relative py-2 ${isActive ? 'text-farm-primary font-bold' : 'text-gray-700 hover:text-farm-primary'
                  }`}
              >
                {item.label}
                {isActive && (
        ...
        <div className="flex items-center gap-4 text-gray-700">
          ...
          <button className="p-2 hover:bg-gray-100 rounded-full transition-colors hidden sm:block btn-auth-trigger"><User size={20} /></button>
          <Suspense fallback={<div className="w-8 h-8 rounded-full bg-gray-100 animate-pulse"></div>}>
            <CartIcon />
          </Suspense>
          <button className="md:hidden p-2 hover:bg-gray-100 rounded-full transition-colors"><Menu size={24} /></button>
        </div>
      </div>
    </header>
  );
...
const Footer = () => {
  ...
  return (
    <footer className="relative text-white/80 pt-20 pb-8 rounded-t-[40px] mt-20 overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-farm-primary-dark">
        <div className="absolute inset-0 opacity-5" style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
          backgroundSize: '60px 60px'
        }}></div>
        {/* Gradient Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-farm-primary-dark/20 to-transparent"></div>
      </div>
        ...
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
          <div className="md:col-span-3">
            <Link to="/" className="flex items-center gap-2 mb-6">
              <Leaf size={32} className="text-white" fill="currentColor" fillOpacity={0.2} />
              <span className="font-serif font-bold text-2xl uppercase tracking-tight text-white">
                {config.brandName}
              </span>
            </Link>
